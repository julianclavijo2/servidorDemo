...
isAuthenticated = ({ email, password , id }) => userdb.users.findIndex(user => user.email === email && user.password === password) !== - 1;

// Creamos un ENDPOINT para comprobar si el usuario existe y poder crear y enviar un TOKEN
server.post('/auth/login', (req, res) => {
    const { email, password , id } = req.body;
    if (isAuthenticated({ email, password , id }) === false) {
        const status = 401;
    ...
    res.status(200).json({ access_token , email , id})
});
